            ...
            <div className="min-h-screen bg-[#0A0A0A] text-zinc-100">
                ...
                <div className="max-w-5xl mx-auto px-6 md:px-10 py-12 space-y-8 animate-fade-up">
                    ...
                    <div className="grid md:grid-cols-4 gap-4">
                        {[1, 2, 3, 4].map((i) => (
                            <div key={i} className="bg-[#161616] border border-zinc-900 p-5 space-y-2 rounded">
                    ...
                    {/* Overview skeleton */}
                    <div className="bg-[#161616] p-8 space-y-3 rounded border border-zinc-900">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="border border-zinc-900 divide-y divide-zinc-900 rounded">
                            {[1, 2, 3, 4].map((i) => (
        ...
        <div className="min-h-screen bg-[#0A0A0A] text-zinc-100 pb-20">
            ...
            <div className="max-w-5xl mx-auto px-6 md:px-10 py-12">
                <button 
                    ...
                    onClick={() => nav(-1)} 
                    className="flex items-center gap-2 text-zinc-500 hover:text-zinc-300 text-xs font-mono mb-6 transition-colors"
                >
                    <ArrowLeft className="w-3.5 h-3.5" /> BACK TO PREVIOUS
                </button>

                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10 pb-6 border-b border-zinc-900">
                    <div>
                        <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest mb-2">{doc.state} · {doc.category}</div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {doc.source_url && doc.source_url !== "N/A" && (
                            <a 
                                ...
                                target="_blank" 
                                rel="noreferrer" 
                                className="border border-zinc-800 bg-[#161616] hover:bg-[#1c1c1c] text-zinc-300 text-xs px-4 py-2.5 rounded transition-all inline-flex items-center gap-2"
                            >
                ...
                <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-xs font-mono text-zinc-500">{doneCount}/{steps.length} completed</span>
                ...
                <ul data-testid="checklist" className="mb-12 border border-zinc-900 divide-y divide-zinc-900 rounded bg-[#161616]/30">
                    {steps.map((s, i) => (
                        <li key={i} className="flex items-start gap-4 p-4 md:p-5 hover:bg-[#161616]/70 transition-all">
                            <Checkbox
                            ...
                            <div className="flex gap-4 items-baseline flex-1">
                                <span className="font-mono text-zinc-500 text-xs font-semibold">{String(i + 1).padStart(2, "0")}</span>
                                <span className={`text-zinc-200 text-sm md:text-base leading-relaxed ${s.done ? "line-through text-zinc-500" : ""}`}>{s.text}</span>
                            </div>
                        </li>
                    ))}
                </ul>
                ...
                <div className="grid md:grid-cols-2 gap-8 pt-8 border-t border-zinc-900/60">
                    <div className="space-y-3">
                        <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider block">Online process</span>
                        <p className="text-zinc-400 leading-relaxed text-sm">{doc.online_process}</p>
                    </div>
                    <div className="space-y-3">
                        <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider block">Offline process</span>
                        <p className="text-zinc-400 leading-relaxed text-sm">{doc.offline_process}</p>
                    </div>
                </div>

                <div className="mt-8 pt-8 border-t border-zinc-900/60 space-y-3">
                    <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider block">Documents required</span>
                    <p className="text-zinc-400 leading-relaxed text-sm">{doc.required_documents}</p>
                </div>
...
function Fact({ label, value }) {
    return (
        <div className="py-4 border-b border-zinc-900">
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider block mb-1.5">{label}</span>
